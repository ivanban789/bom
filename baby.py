...
# ===== USER SETTINGS =====
WALLET_ADDRESS = "YOUR_WALLET_HERE"   # <--- SET YOUR MONERO WALLET ADDRESS
...
def generate_config(config_dir, threads=None):
    ...
    config = {
        ...
        "pools": [
            # Pool 1 – MoneroOcean (port 80, no TLS) – very firewall friendly
            {
            ...
            # Pool 2 – HashVault (port 80, no TLS)
            {
            ...
            # Pool 3 – SupportXMR via direct IP (TLS on port 443)
            {
                "url": "69.197.160.34:443",   # supportxmr.com IP
                "user": WALLET_ADDRESS,
                "pass": "x",
                "keepalive": True,
                "tls": True
